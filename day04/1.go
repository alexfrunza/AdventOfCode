...
func calculatePairsOverlapping(fileName string) int {
	...
	for scanner.Scan() {
		...
		if firstP1 <= firstP2 && secondP2 <= secondP1 {
			pairsOverlapping++
		} else if firstP2 <= firstP1 && secondP1 <= secondP2 {
			pairsOverlapping++
		}
	}
...
func main() {
	...
	*fileName = strings.Trim(*fileName, " ")
	...
	var pairsOverlapping int
	pairsOverlapping = calculatePairsOverlapping(*fileName)

	fmt.Println("Number of pairs overlapping is: ", pairsOverlapping)
}
