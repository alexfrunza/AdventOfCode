...
func throwItem(m *Monkey, monkeys []Monkey, gDivisor int) {
	if m.items.first.info%m.testDivisor == 0 {
		addItem(&monkeys[m.trueNumber].items, m.items.first.info%gDivisor)
		removeItem(&m.items)
	} else {
		addItem(&monkeys[m.falseNumber].items, m.items.first.info%gDivisor)
		removeItem(&m.items)
	}
}
